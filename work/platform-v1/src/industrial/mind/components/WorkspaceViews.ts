...
export class MINDWorkspaceEngine {
  public renderLevel1TrainingBrain(activeHole: any, currentDepth: number): string {
    const nearbyHazards = activeHole.intervals
      .filter((inv: any) => inv.lessonsLearned?.hazardFlagged && Math.abs(inv.toMeter - currentDepth) <= 20)
      .map((inv: any) => `?? DEPTH [${inv.fromMeter}m-${inv.toMeter}m]: ${inv.lessonsLearned?.notes}`);
...
================================================================================`;
  ...
  public renderLevel2OperationsBrain(activeHole: any): string {
    const totalIntervals = activeHole.intervals.length;
    const totalDowntime = activeHole.intervals.reduce((acc: number, inv: any) => acc + inv.operations.downtimeMinutes, 0);
...
${activeHole.intervals.map((inv: any) => 
...
================================================================================`;
  ...
  public renderLevel3ExecutiveBrain(activeHole: any): string {
    const totals = activeHole.intervals.reduce((acc: any, current: any) => {
...
================================================================================`;
  ...
  public routeToWorkspace(session: any, activeHole: any): void {
