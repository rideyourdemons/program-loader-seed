...
(function() {
  ...
  // Ensure dataLayer exists early (GTM processes queued events when it loads)
  ...
  // Initialize GTM
  function initGTM() {
    // Skip in local dev unless explicitly enabled (keeps dev analytics-free by default)
    ...
    // Production: use main container; local+debug: use test container if set
    const containerId = isLocal ? (GTM_TEST_ID || null) : (GTM_CONTAINER_ID || null);
  ...
  // Push event to dataLayer (always push; GTM processes queued events when loaded)
  ...
  // GA4-style event (used by insights, tools, etc.) – pushes to dataLayer for GTM/GA4
  ...
  // Global helper (insights.html, tools, etc. call this)
