...
// RYD CORE GENERATOR LOGIC
const generateTool = (domain, toolName, tacticalWhy, source, stateId = 'S1') => {
    ...
    const newTool = {
        ...
        title: toolName,
        slug: toolId,
        action: `${tacticalWhy}. This tactic works by creating an intentional interruption in automatic response patterns.`,
        description: expandedDescription,
        // Mental Mechanics Bridge
        how_it_works: expandedHowItWorks,
        where_it_came_from: expandedWhereItCameFrom,
        steps: [
            "Isolate the systemic trigger.",
    ...
    if (!data.tools.find(t => t.id === toolId || t.name === toolName)) {
        data.tools.push(newTool);
        fs.writeFileSync(filePath, JSON.stringify(data, null, 2) + '\n');
        console.log(`✅ Integrated Tactical Tool: ${toolName} in ${mappedDomain}`);
    } else {
        console.log(`⏭️  Tool already exists: ${toolName} in ${mappedDomain}`);
    }
};

// THE SEED LIST - REAL WORLD INTEL
const SEED_LIST = [
    ...
    { domain: 'burnout-and-resilience', name: 'The Load Reset', why: 'Forces a system blackout to flush cortisol', source: 'Stress Response Research' },
    { domain: 'conflict-tactics', name: 'Ego De-escalation', why: 'Manually suppresses the social-threat response', source: 'Evolutionary Psychology' },
    { domain: 'burnout-and-resilience', name: 'Decision Fatigue Bypass', why: 'Automates low-stakes choices to preserve focus', source: 'Cognitive Load Theory' },
    { domain: 'loss-and-grief', name: 'Structural Recalibration', why: 'Maps loss as a utility failure to enable rebuilding', source: 'Traumatic Grief Mechanics' }
];

console.log('--- 🚀 STARTING PROGRAMMATIC RYD EXPANSION ---');
SEED_LIST.forEach(item => generateTool(item.domain, item.name, item.why, item.source));
console.log('--- 🛡️ EXPANSION COMPLETE. RUNNING VALIDATOR... ---');
