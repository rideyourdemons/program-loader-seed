...
function processTools() {
  ...
  if (missing.length > 0 && missing.length <= 10) {
    console.log(`\nTools with missing fields (first 10):`);
    missing.slice(0, 10).forEach(m => {
      console.log(`  - ${m.slug}: missing ${m.missing.join(', ')}`);
    });
  } else if (missing.length > 10) {
    console.log(`\n${missing.length} tools still have missing fields (showing first 10)`);
    missing.slice(0, 10).forEach(m => {
      console.log(`  - ${m.slug}: missing ${m.missing.join(', ')}`);
    });
  }
  ...
  return { total: tools.length, updated, filled, missing: missing.length };
}
